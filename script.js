...
$(document).ready(function(){
	$(".window").each(function() {      		// window
		...
		$("#taskbar").append('<div class="taskbarPanel" id="minimPanel' + i + '" data-id="' + i + '"> <img src="exe.png" class="taskbar-icon"> ' + $(this).attr("data-title") + '</div>');
		if ($(this).hasClass("closed")) {	$("#minimPanel" + i).addClass('closed');	}		
		$(this).attr('id', 'window' + (i++));
		$(this).wrapInner('<div class="wincontent"></div>');
		// no float design $(this).prepend('<div class="windowHeader">' + $(this).attr("data-title") + '<span title="Minimize" class="winminimize">-</span><span title="Maximize" class="winmaximize">&#9633;</span><span title="Close" class="winclose">x</span></div>');
		$(this).prepend('<div class="windowHeader"><img src="exe.png" class="winicon">&nbsp;&nbsp;<i>' + $(this).attr("data-title") + '</i><span title="Close" class="winclose"><z>🞨</z></span><span title="Maximize" class="winmaximize"><strong>&#9633;</strong></span><span title="Minimize" class="winminimize">⎯</span></div>');
	});
	...
	$("#app-menu .openWindow").click(function() {
        openWindow($(this).attr("data-id"));
    });
